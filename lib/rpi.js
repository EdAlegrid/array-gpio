...
var rpiSetup = { initialized: false, gpiomem: false };
...
function rpiModeConflict(){
  console.log('\n** It seems a GPIO peripheral is already using the rpi library in dev/gpiomem.');
  console.log('** PWM, I2C and SPI requires the rpi library to be initialized in dev/mem.');
  console.log('** Try creating your PWM/I2C/SPI objects before the GPIO input/output objects.\n');
}
...
open (pin, mode, init)
{
  ...
  if(mode === this.INPUT){ 	/* set as INPUT */
  ...
  else if(mode === this.OUTPUT){ 	/* set as OUTPUT */
...
pwmInit ()
{
    ...
    rpiModeConflict();
		throw new Error('peripheral mode access conflict');
  } 
...
i2cBegin ()
{
 	if (rpiSetup.initialized && rpiSetup.gpiomem){
  	rpiModeConflict();
		throw new Error('peripheral access conflict');
  }
