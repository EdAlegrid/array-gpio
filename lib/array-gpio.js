...
/*
 * watch/monitor all gpio input pins
 */
function watchPin(edge, cb, pin, td){
	...
	rpi.gpio_enable_pud(pin, rpi.PULL_DOWN)
	...
	rpi.gpio_reset_all_pin_events(pin);
	rpi.gpio_enable_async_rising_pin_event(pin);

	function logic () {
		if(rpi.gpio_detect_input_pin_event(pin) && !on){
			on = true;
			if(edge === 1 || edge === 're' || edge === 'both' ||  edge === null ){
				setImmediate(cb, true, pin);
			}
			rpi.gpio_reset_pin_event(pin);
		}
		else if(!rpi.gpio_detect_input_pin_event(pin) && on){
			on = false;
			if(edge === 0 || edge === 'fe' || edge == 'both' || edge === null){  
				setImmediate(cb, false, pin);
			}
...
/* unwatch a particular input pin */
